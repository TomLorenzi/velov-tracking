...
async function updateStations() {
  ...
  try {
    const liveStations = await fetch(`https://api.jcdecaux.com/vls/v3/stations?apiKey=${process.env.NEXT_PUBLIC_VELOV_API_KEY}&contract=lyon`);
    ...
    if (listStationToInsert.length) {
      await prisma.station.createMany({data: listStationToInsert});
    }
  } catch (e) {
...
async function updateLiveStations() {
  if (process.env.NEXT_PUBLIC_VELOV_API_KEY === undefined) {
  ...
  try {
    ...
    console.error("Error updating live stations:", e);
    return;
  }
}
...
async function updateBikes() {
  ...
  for (const bike of liveBikes) {
    if (!["AVAILABLE", "RENTED", "RESERVED"].includes(bike.status) || (undefined === liveUpdatedStations[bike.stationNumber] && bike.status !== "RENTED")) {
      continue;
    }
    ...
    if (formattedBikes[bike.number] !== undefined) {
      ...
      listBikesToInsert.push({number: bike.number, type: bike.type, stationNumber: bike.stationNumber});
    }
  }
  try {
    if (listBikesToUpdates.length) {
      ...
      await prisma.$transaction([...listBikesToUpdates.map((bike) => prisma.bike.update({where: bike.where, data: bike.data}))]);
    }
    if (listBikesToInsert.length) {
      await prisma.bike.createMany({data: listBikesToInsert});
    }
  } catch (error) {
  ...
  try {
    if (listTravelToInsert.length) {
      await prisma.travel.createMany({data: listTravelToInsert});
    }
    if (listTravelToUpdates.length) {
      await prisma.$transaction([...listTravelToUpdates.map((travel) => prisma.travel.update({where: travel.where, data: travel.data}))]);
    }
  } catch (error) {
...
async function updateTravel(endStationNumber : number | null, bike : PrismaBike, listTravelToInsert : any[], listTravelToUpdates : any[]) {
  ...
  if (endStationNumber === null) {
    listTravelToInsert.push({stationFromNumber: bike.stationNumber, bikeNumber: bike.number, startDateTime: new Date()});
...
console.log("Unfinished travels cleaned");
